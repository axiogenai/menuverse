...
export function Navbar() {
  ...
  const [isOwnerHost, setIsOwnerHost] = useState(true);
    ...
    <>
          ...
          <div className="flex items-center gap-2">
            {!isDashboard && (
              <Link
                href="/dashboard"
                prefetch={true}
                className="flex items-center gap-1.5 px-3 py-1.5 sm:px-3.5 sm:py-2 rounded-xl bg-slate-900 hover:bg-slate-800 text-white text-xs font-semibold shadow-xs transition-all active:scale-95 touch-manipulation cursor-pointer"
              >
            ...
            <button
              ...
              className="md:hidden p-2 rounded-xl text-slate-700 hover:text-slate-900 hover:bg-slate-100 transition-colors border border-slate-200 shadow-2xs cursor-pointer touch-manipulation"
      ...
      {mounted && mobileMenuOpen && createPortal(
        <div className="fixed inset-0 z-[99999] md:hidden">
          {/* Dimmed Backdrop */}
          <div 
            className="fixed inset-0 bg-black/60 backdrop-blur-xs transition-opacity animate-in fade-in duration-150 touch-manipulation"
            onClick={() => setMobileMenuOpen(false)}
          />

          {/* Side Drawer Panel */}
          <div className="fixed inset-y-0 right-0 w-80 max-w-[85vw] h-full bg-white shadow-2xl flex flex-col z-50 animate-in slide-in-from-right duration-200">
            {/* Minimal Drawer Top Bar */}
            <div className="h-14 px-4 flex items-center justify-between shrink-0 bg-white border-b border-slate-100">
              <div className="flex items-center gap-2">
                <Crown className="w-4 h-4 text-amber-500" />
                <span className="text-xs font-bold text-slate-900">Hotel Gypsy</span>
              </div>
              <button
                type="button"
                onClick={() => setMobileMenuOpen(false)}
                className="p-2 rounded-xl text-slate-500 hover:text-slate-900 hover:bg-slate-100 transition-colors border border-slate-200 cursor-pointer touch-manipulation"
            ...
            <div className="flex-1 overflow-y-auto p-4 space-y-4 bg-slate-50/70">
              {/* CASE 1: USER IS ON PUBLIC LIVE MENU -> SHOW DINER NAVIGATION */}
              {!isDashboard ? (
                <div className="space-y-3">
                  ...
                  <div className="bg-white rounded-2xl border border-slate-200/90 shadow-2xs divide-y divide-slate-100 overflow-hidden">
                    <Link
                      ...
                      prefetch={true}
                      onClick={() => setMobileMenuOpen(false)}
                      className="flex items-center justify-between p-3.5 hover:bg-slate-50 active:bg-slate-100 transition-colors touch-manipulation cursor-pointer"
                    >
                    ...
                    {managementNav.map((item) => {
                      ...
                      return (
                        <Link
                          ...
                          onClick={() => setMobileMenuOpen(false)}
                          className={cn(
                            "flex items-center justify-between p-3 transition-colors touch-manipulation cursor-pointer",
                            isActive ? "bg-slate-900 text-white font-semibold" : "hover:bg-slate-50 active:bg-slate-100 text-slate-700"
                          )}
                        >
            ...
            {/* Bottom Context Action - Fast Switcher Between Public Menu & Owner Portal */}
            <div className="p-3.5 border-t border-slate-200 bg-white shrink-0">
              {isDashboard ? (
                <Link
                  ...
                  prefetch={true}
                  onClick={() => setMobileMenuOpen(false)}
                  className="w-full py-2.5 px-3 rounded-xl bg-orange-600 hover:bg-orange-500 text-white text-xs font-bold flex items-center justify-center gap-1.5 shadow-sm transition-all active:scale-95 touch-manipulation cursor-pointer"
                >
                ...
                <Link
                  ...
                  prefetch={true}
                  onClick={() => setMobileMenuOpen(false)}
                  className="w-full py-2.5 px-3 rounded-xl bg-slate-900 hover:bg-slate-800 text-white text-xs font-bold flex items-center justify-center gap-1.5 shadow-sm transition-all active:scale-95 touch-manipulation cursor-pointer"
                >
